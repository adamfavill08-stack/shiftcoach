...
import { createTask, updateEvent } from '@/lib/helpers/calendar/EventsHelper'
import { useTranslation } from '@/components/providers/language-provider'
import { cancelTaskReminders, scheduleTaskReminders } from '@/lib/notifications/taskLocalNotifications'
...
export function TaskFormModal({ isOpen, onClose, task, defaultDate, onSave }: TaskFormModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.title?.trim()) {
      alert(t('calendar.taskForm.alertTitleRequired'))
      return
    }
    ...
    try {
      let saved = null
      if (task?.id) {
        saved = await updateEvent(task.id, formData)
      } else {
        saved = await createTask(formData)
      }

      const id = saved?.id ?? task?.id
      if (id != null) {
        await cancelTaskReminders(id)
        const startSec = saved?.startTS ?? formData.startTS ?? Math.floor(Date.now() / 1000)
        const title =
          (saved?.title ?? formData.title ?? '').trim() || t('calendar.taskForm.defaultTaskName')
        const description = saved?.description ?? formData.description ?? ''
        await scheduleTaskReminders({
          taskId: id,
          title,
          description,
          startSec,
          titleBefore1h: t('calendar.taskNotify.before1h'),
          titleAtStart: t('calendar.taskNotify.atStart'),
        })
      }
      ...
      alert(t('calendar.taskForm.alertSaveFailed'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div
      data-shiftcoach-task-form-root
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {task ? t('calendar.taskForm.editTitle') : t('calendar.taskForm.newTitle')}
              </h2>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                {t('calendar.taskForm.titleLabel')}
              </label>
              <input
                type="text"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                placeholder={t('calendar.taskForm.titlePlaceholder')}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                ...
                {t('calendar.taskForm.notesLabel')}
              </label>
              <textarea
                ...
                placeholder={t('calendar.taskForm.notesPlaceholder')}
              />
            </div>
            ...
            <p className="text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
              {t('calendar.taskForm.reminderHint')}
            </p>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200/50 dark:border-slate-700/40">
              ...
              >
                {loading
                  ? t('calendar.taskForm.saving')
                  : task
                    ? t('calendar.taskForm.update')
                    : t('calendar.taskForm.create')}
              </button>
            </div>
          </form>
        </div>
