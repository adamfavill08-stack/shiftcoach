...
const nextConfig: NextConfig = {
  ...
  // Do not set `turbopack.root` to the monorepo parent — that makes Turbopack use the
  // sparse repo-root `app/` and breaks most `/api/*` routes (404). Default resolution
  // with `next dev apps/web` / `next build apps/web` is correct.
