...
export default function SleepPage() {
  ...
  const fetchSleepData = useCallback(async () => {
    try {
      ...
      console.log('[SleepPage] /api/sleep/summary lastNight:', {
        totalMinutes: lastNight?.totalMinutes ?? null,
        startAt: lastNight?.startAt ?? null,
        endAt: lastNight?.endAt ?? null,
  ...
  useEffect(() => {
    ...
    const handleWearablesSynced = () => {
      // When the user triggers a wearable sync, refresh sleep immediately.
      console.log('[SleepPage] wearables-synced -> dispatch sleep-refreshed')
      window.dispatchEvent(new CustomEvent('sleep-refreshed'))
    }

    const handleWearDataReceived = (event: Event) => {
      const now = Date.now()
      if (now - lastWearableSleepRefreshRef.current < 15000) return // throttle refreshes

      try {
        const detail = (event as CustomEvent).detail
        // Android bridge sends JSON string as `event.detail`
        const parsed = typeof detail === 'string' ? JSON.parse(detail) : detail
        if (!parsed || typeof parsed !== 'object') return

        const text = JSON.stringify(parsed).toLowerCase()
        const looksLikeSleep =
          text.includes('sleep_records') ||
          text.includes('sleep_logs') ||
          text.includes('stages') ||
          text.includes('sleepminutes') ||
          text.includes('sleep_minutes') ||
          text.includes('isasleep') ||
          text.includes('sleepstage') ||
          text.includes('sleep_stage') ||
          text.includes('deep') ||
          text.includes('rem') ||
          text.includes('light') ||
          text.includes('awake') ||
          text.includes('start_at') ||
          text.includes('end_at') ||
          text.includes('start_ts') ||
          text.includes('end_ts')

        if (!looksLikeSleep) return

        lastWearableSleepRefreshRef.current = now
        console.log('[SleepPage] wearDataReceived (sleep payload) -> dispatch sleep-refreshed', {
          keys: Object.keys(parsed),
          recordedAt: (parsed as any).recordedAt ?? null
        })
        window.dispatchEvent(new CustomEvent('sleep-refreshed'))
      } catch (err) {
        // Avoid breaking the UI if the payload isn't JSON
      }
    }

    window.addEventListener('sleep-refreshed', handleSleepRefresh)
    window.addEventListener('wearables-synced', handleWearablesSynced as EventListener)
    window.addEventListener('wearDataReceived', handleWearDataReceived as EventListener)
    
    return () => {
      window.removeEventListener('sleep-refreshed', handleSleepRefresh)
      window.removeEventListener('wearables-synced', handleWearablesSynced as EventListener)
      window.removeEventListener('wearDataReceived', handleWearDataReceived as EventListener)
    }
  }, [fetchSleepData])
