...
export default function ActivityAndStepsPage() {
  ...
  const [watchSteps, setWatchSteps] = useState<number>(0)

  const parseWatchSteps = (detail: any): number | null => {
    if (detail == null) return null

    // Bridge sometimes sends a raw JSON string.
    ...
    // Handle common payload shapes: { steps }, { sample: { steps } }, { data: { steps } }, { detail: { steps } }
    const candidates = [
      (payload as any).steps,
      (payload as any).sample?.steps,
      (payload as any).data?.steps,
      (payload as any).detail?.steps,
    ]
  ...
  // Listen for real-time watch data coming from the native Android bridge
  useEffect(() => {
    if (typeof window === 'undefined') return

    const handleWatchSync = (event: Event) => {
      try {
        const detail = (event as CustomEvent).detail
        const steps = parseWatchSteps(detail)
        if (steps != null) {
          console.log('[ActivityAndStepsPage] Real-time Watch Steps:', steps, detail)
          setWatchSteps(steps)
          setLastWearableSync(Date.now())
          window.localStorage.setItem('wearables:lastSyncedAt', String(Date.now()))
          window.localStorage.setItem('wearables:lastSampleSteps', String(steps))
          // If you want to refresh server-derived activity as well, you could call:
          // mutate('/api/activity/today')
        }
      } catch (e) {
        console.error('[ActivityAndStepsPage] Error parsing watch data from wearDataReceived', e)
      }
    }

    // Seed from localStorage in case event arrived before this component mounted.
    const cachedSteps = window.localStorage.getItem('wearables:lastSampleSteps')
    if (cachedSteps && !Number.isNaN(Number(cachedSteps))) {
      setWatchSteps(Number(cachedSteps))
    }

    window.addEventListener('wearDataReceived', handleWatchSync as EventListener)
    return () => window.removeEventListener('wearDataReceived', handleWatchSync as EventListener)
  }, [])
  ...
  const displaySteps = Math.max(0, watchSteps, data.steps ?? 0)
  ...
  const activitySourceLabel = useMemo(() => {
    if (watchSteps > 0) return 'Source: Wear OS (live)'
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                {watchSteps > 0 && (
                  <span className="rounded-full bg-emerald-50/80 dark:bg-emerald-900/40 border border-emerald-200/70 dark:border-emerald-700/60 px-2.5 py-1 text-[11px] text-emerald-700 dark:text-emerald-200">
                    Watch steps: {watchSteps.toLocaleString()}
                  </span>
                )}
              </div>
            </div>
