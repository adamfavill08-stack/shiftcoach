  ...
  const refetch = async (forceFresh = false) => {
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          console.warn(
            '[useTodayNutrition] 404 on /api/nutrition/today — API route missing. From repo root run `npm run dev` (uses `next dev apps/web`), not plain `next dev`. Clear apps/web/.next if it persists.',
          )
        } else {
