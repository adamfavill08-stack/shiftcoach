...
export default function SleepLogsPage() {
  ...
  const getShiftColor = (label: string) => {
    ...
    if (normalized.includes('night')) return 'bg-slate-700 dark:bg-slate-600 text-white dark:text-white'
    if (normalized.includes('day') || normalized.includes('morning') || normalized.includes('afternoon')) return 'bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 border border-blue-200/60 dark:border-blue-800/40'
    if (normalized.includes('off') || normalized.includes('rest')) return 'bg-slate-100 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/40'
    return 'bg-slate-200 dark:bg-slate-800/50 text-slate-800 dark:text-slate-300 border border-slate-200/60 dark:border-slate-700/40'
  }
      ...
      <div className="px-4 py-4 space-y-3">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="h-20 bg-slate-100/60 dark:bg-slate-800/50 rounded-xl animate-pulse" />
            ))}
          </div>
        ) : days.length === 0 ? (
          ...
          days.map((day) => (
            <div
              key={day.date}
              className="relative overflow-hidden rounded-xl bg-white/90 dark:bg-slate-900/45 backdrop-blur-xl border border-white dark:border-slate-700/40 shadow-[0_12px_40px_rgba(15,23,42,0.06)] dark:shadow-[0_24px_60px_rgba(0,0,0,0.5)]"
            >
              {/* Premium gradient overlay */}
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/85 dark:from-slate-900/70 to-white/55 dark:to-slate-950/60" />
              
              <div className="relative z-10 p-3.5">
                {/* Day Header */}
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100/80 dark:bg-slate-800/50 border border-slate-200/60 dark:border-slate-700/40">
                      <CalendarIcon className="h-4 w-4 text-slate-600 dark:text-slate-400" strokeWidth={2} />
                    </div>
                    <div>
                ...
                {day.logs.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {day.logs.map((log) => (
                      <div
                        key={log.id}
                        className="group relative overflow-hidden rounded-lg border border-slate-200/80 dark:border-slate-700/40 bg-white/90 dark:bg-slate-800/50 backdrop-blur-sm px-3 py-2.5 transition-all duration-200 hover:shadow-sm dark:hover:shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:border-slate-300/80 dark:hover:border-slate-600/60 cursor-pointer"
                        onClick={() => handleEdit(log)}
                      >
                        {/* Subtle gradient overlay */}
                        <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-white/60 dark:from-slate-800/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
                          ...
                          <div className="flex items-center gap-2.5 flex-1 min-w-0">
                            ...
                            `}>
                              {log.type === 'sleep' ? (
                                ...
                                <Coffee className="h-4 w-4 text-amber-600 dark:text-amber-400" strokeWidth={2} />
                              )}
                            </div>
