...
export default function StepsPage(_props: Props) {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      let { data: weekActivities, error: weekActivitiesError } = await supabase
        .from('activity_logs')
        .select('steps, logged_at, created_at')
        .eq('user_id', user.id)
      ...
      if (weekActivities && weekActivities.length > 0) {
        ...
        weekActivities.forEach((activity: any) => {
          const date = new Date(activity.logged_at || activity.created_at).toISOString().slice(0, 10)
        ...
        // If logged_at isn't present, fall back to created_at only
        if (weekActivitiesError) {
          const { data: weekActivitiesTs } = await supabase
            .from('activity_logs')
          ...
          if (weekActivitiesTs && weekActivitiesTs.length > 0) {
            const daysMap = new Map<string, number>()
            weekActivitiesTs.forEach((activity: any) => {
  ...
  const handleAddManualSteps = async () => {
    ...
    try {
      ...
      let existingQuery = await supabase
        .from('activity_logs')
        .select('id, steps, logged_at_timestamp, created_at, source')
        .eq('user_id', user.id)
        .gte('logged_at_timestamp', startOfDay.toISOString())
        .lt('logged_at_timestamp', endOfDay.toISOString())
        .order('logged_at_timestamp', { ascending: false })
        .limit(1)
        .maybeSingle()

      if (existingQuery.error && (existingQuery.error.code === '42703' || existingQuery.error.message?.includes('logged_at_timestamp'))) {
      ...
      if (existing) {
        ...
        let insertQuery = await supabase
          .from('activity_logs')
          .insert({ ...insertData, logged_at_timestamp: new Date().toISOString() })
          .select('steps')
          .single()
