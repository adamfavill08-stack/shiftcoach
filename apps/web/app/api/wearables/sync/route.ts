...
export async function POST(req: NextRequest) {
  try {
    ...
    // Shift workers: aggregate Google Fit steps over the rota shift window:
    // shift start - 1h -> shift end + 1h (capped to now).
    ...
    // Fallback window if shifts aren't configured
    ...
    let currentShift = (shiftInProgress as any) ?? null
    if (!currentShift) {
      // Fallback: last started shift (covers cases where end_ts isn't set)
    ...
    const windowStartDate = currentShift?.start_ts
      ? new Date(new Date(currentShift.start_ts).getTime() - SHIFT_WINDOW_BUFFER_MS)
      : fallbackStartOfDay

    let windowEndDate = currentShift?.end_ts
    ...
    console.log('[wearables/sync] Google Fit total steps (shift window):', totalSteps)

    // Upsert the shift-window activity_logs entry
    ...
    if (!existingQuery.error && existingQuery.data) {
      ...
      let insertQuery = await supabase
        ...
        .insert({
          ...
          steps: totalSteps,
          source: sourceLabel,
          // Keep ts stable for this shift so we update the same row
          // across midnight.
          ts: startIso,
        })
        .select('id')
