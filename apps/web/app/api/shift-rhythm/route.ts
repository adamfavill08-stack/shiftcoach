...
export async function buildShiftRhythmInputs(supabase: SupabaseClient, userId: string) {
  ...
  const startOfDayIso = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate(), 0, 0, 0)).toISOString()
  const endOfDayIso = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate() + 1, 0, 0, 0)).toISOString()
  ...
  let activityResponse = await supabase
    .from('activity_logs')
    .select('steps,minutes_active,source,logged_at')
    .eq('user_id', userId)
  ...
  let activeMinutes: number | null = activityResponse.data?.minutes_active ?? null

  if (activityResponse.error) {
    const err = activityResponse.error
    if (err.code === '42703' || err.message?.includes('minutes_active')) {
      console.warn('[/api/shift-rhythm] activity_logs.minutes_active missing, falling back without it')
